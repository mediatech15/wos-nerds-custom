...
const DEFAULT_TEAMS = Object.freeze([
  ...
  Object.freeze({ name: 'Counters', color: '#f7ae28' }),
  Object.freeze({ name: 'Turrets', color: '#ef4494' })
])
...
function addEntity (event) {
  ...
  let color; let width; let height; let id = null
  if (selectedType === 'flag') {
    ...
    // if (enemyZones.length >= 5) {
    //     alert('You can only place up to 5 Enemy Zones.');
    //     return;
    // }
    color = 'black'
    width = 2
...
function handleKeyDown (event) {
  ...
  if (modKey && event.key && event.key.toLowerCase() === 'c') {
    event.preventDefault()
  }

  if (modKey && event.key && event.key.toLowerCase() === 'v') {
    event.preventDefault()
  }
...
function compressMapWithName (entities, mapName, anchor = coordAnchor, _waveMode = waveMode, _cityLabelMode = cityLabelMode, _mapMode = mapMode) {
  ...
  const base64String = compressMap(serializableEntities)
  ...
  for (let r = 0; r < rows.length; r++) {
    ...
    if (existingCity) {
      ...
      if (moveDefaultCities && Number.isFinite(rec.x) && Number.isFinite(rec.y)) {
        const width = existingCity.width || 2; const height = existingCity.height || 2
...
(function () {
  ...
  panels.forEach(panel => {
    let startY = 0; let curY = 0; let isDragging = false
